...
const RescuePetData = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`https://pawkie-server.vercel.app/api/rescue-posts/${id}`);
  ...
  return (
    <div className="p-6 bg-[#FFE8DA] min-h-screen">
      <h2 className="text-3xl font-bold text-[#5F040D] mb-6 text-center">Your Rescue Pet Posts</h2>

      {posts.length === 0 ? (
        <p className="text-center text-gray-600">No rescue posts found.</p>
      ) : (
            ...
            <tbody>
              {posts.map(post => (
                <tr key={post._id} className="border-t border-[#FFE8DA] hover:bg-[#FFF3EC] transition">
                  <td className="p-3">
                    <img src={post.image} alt="Rescue Pet" className="w-20 h-20 object-cover rounded-md" />
                  </td>
                  <td className="p-3 whitespace-pre-line text-gray-700">{post.details}</td>
